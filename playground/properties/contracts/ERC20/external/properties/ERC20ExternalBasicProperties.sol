...
abstract contract CryticERC20ExternalBasicProperties is CryticERC20ExternalTestBase {
    ...
    function test_ERC20external_userBalancesLessThanTotalSupply() public {
        uint256 sumBalances =
            token.balanceOf(address(this)) + token.balanceOf(USER1) + token.balanceOf(USER2) + token.balanceOf(USER3);
    ...
    // Transfers to zero address should not be allowed
    function test_ERC20external_transferFromToZeroAddress(uint256 value) public {
        ...
        bool r = token.transferFrom(msg.sender, address(0), value % (maxValue + 1));
    ...
    function test_ERC20external_selfTransferFrom(uint256 value) public {
        ...
        bool r = token.transferFrom(msg.sender, msg.sender, value % (maxValue + 1));
        assertWithMsg(r == true, "Failed self transferFrom");
        assertEq(balance_sender, token.balanceOf(msg.sender), "Self transferFrom breaks accounting");
    }
    ...
    function test_ERC20external_selfTransfer(uint256 value) public {
        ...
        assertEq(balance_sender, token.balanceOf(address(this)), "Self transfer breaks accounting");
    }

    // Transfers for more than available balance should not be allowed
    function test_ERC20external_transferFromMoreThanBalance(address target) public {
        uint256 balance_sender = token.balanceOf(msg.sender);
        ...
        assertWithMsg(r == false, "Successful transferFrom for more than account balance");
        assertEq(
            token.balanceOf(msg.sender), balance_sender, "TransferFrom for more than balance modified source balance"
        );
        assertEq(
            token.balanceOf(target), balance_receiver, "TransferFrom for more than balance modified target balance"
        );
    }
    ...
    function test_ERC20external_transferMoreThanBalance(address target) public {
        ...
        assertWithMsg(r == false, "Successful transfer for more than account balance");
        assertEq(
            token.balanceOf(address(this)), balance_sender, "Transfer for more than balance modified source balance"
        );
        assertEq(token.balanceOf(target), balance_receiver, "Transfer for more than balance modified target balance");
    }
    ...
    function test_ERC20external_transferZeroAmount(address target) public {
        ...
        assertEq(token.balanceOf(address(this)), balance_sender, "Zero amount transfer modified source balance");
        assertEq(token.balanceOf(target), balance_receiver, "Zero amount transfer modified target balance");
    }
    ...
    function test_ERC20external_transferFromZeroAmount(address target) public {
        ...
        assertEq(token.balanceOf(msg.sender), balance_sender, "Zero amount transferFrom modified source balance");
        assertEq(token.balanceOf(target), balance_receiver, "Zero amount transferFrom modified target balance");
    }

    // Transfers should update accounting correctly
    function test_ERC20external_transfer(address target, uint256 amount) public {
    ...
    // Transfers should update accounting correctly
    function test_ERC20external_transferFrom(address target, uint256 amount) public {
        require(target != address(this));
        ...
        assertWithMsg(r == true, "transfer failed");
        assertEq(
            token.balanceOf(msg.sender), balance_sender - transfer_value, "Wrong source balance after transferFrom"
        );
        assertEq(token.balanceOf(target), balance_receiver + transfer_value, "Wrong target balance after transferFrom");
    }

    // Approve should set correct allowances
    function test_ERC20external_setAllowance(address target, uint256 amount) public {
        bool r = token.approve(target, amount);
        assertWithMsg(r == true, "Failed to set allowance via approve");
        assertEq(token.allowance(address(this), target), amount, "Allowance not set correctly");
    }

    // Approve should set correct allowances
    function test_ERC20external_setAllowanceTwice(address target, uint256 amount) public {
        bool r = token.approve(target, amount);
        assertWithMsg(r == true, "Failed to set allowance via approve");
        assertEq(token.allowance(address(this), target), amount, "Allowance not set correctly");
        ...
        assertWithMsg(r == true, "Failed to set allowance via approve");
        assertEq(token.allowance(address(this), target), amount / 2, "Allowance not set correctly");
    }

    // TransferFrom should decrease allowance
    function test_ERC20external_spendAllowanceAfterTransfer(address target, uint256 amount) public {
