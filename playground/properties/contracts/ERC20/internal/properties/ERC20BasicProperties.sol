...
abstract contract CryticERC20BasicProperties is CryticERC20Base {
    ...
    function test_ERC20_transferFromToZeroAddress(uint256 value) public {
        ...
        uint256 maxValue = balance_sender >= current_allowance ? current_allowance : balance_sender;
    ...
    function test_ERC20_selfTransferFrom(uint256 value) public {
        ...
        assertEq(balance_sender, balanceOf(msg.sender), "Self transferFrom breaks accounting");
    }
    ...
    function test_ERC20_transferFromMoreThanBalance(address target) public {
        ...
        assertEq(balanceOf(msg.sender), balance_sender, "TransferFrom for more than balance modified source balance");
        assertEq(balanceOf(target), balance_receiver, "TransferFrom for more than balance modified target balance");
    }
    ...
    function test_ERC20_transferMoreThanBalance(address target) public {
        ...
        assertEq(balanceOf(address(this)), balance_sender, "Transfer for more than balance modified source balance");
        assertEq(balanceOf(target), balance_receiver, "Transfer for more than balance modified target balance");
    }
    ...
    function test_ERC20_transferZeroAmount(address target) public {
        ...
        assertEq(balanceOf(address(this)), balance_sender, "Zero amount transfer modified source balance");
        assertEq(balanceOf(target), balance_receiver, "Zero amount transfer modified target balance");
    }
    ...
    function test_ERC20_transferFromZeroAmount(address target) public {
        ...
        assertEq(balanceOf(msg.sender), balance_sender, "Zero amount transferFrom modified source balance");
        assertEq(balanceOf(target), balance_receiver, "Zero amount transferFrom modified target balance");
    }
    ...
    function test_ERC20_transferFrom(address target, uint256 amount) public {
        ...
        assertEq(balanceOf(msg.sender), balance_sender - transfer_value, "Wrong source balance after transferFrom");
        assertEq(balanceOf(target), balance_receiver + transfer_value, "Wrong target balance after transferFrom");
    }
    ...
    function test_ERC20_setAllowance(address target, uint256 amount) public {
        ...
        assertEq(allowance(address(this), target), amount, "Allowance not set correctly");
    }

    // Approve should set correct allowances
    function test_ERC20_setAllowanceTwice(address target, uint256 amount) public {
        bool r = this.approve(target, amount);
        assertWithMsg(r == true, "Failed to set allowance via approve");
        assertEq(allowance(address(this), target), amount, "Allowance not set correctly");
        ...
        assertWithMsg(r == true, "Failed to set allowance via approve");
        assertEq(allowance(address(this), target), amount / 2, "Allowance not set correctly");
    }
