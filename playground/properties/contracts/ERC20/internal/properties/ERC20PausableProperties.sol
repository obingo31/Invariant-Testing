...
abstract contract CryticERC20PausableProperties is CryticERC20Base, ERC20Pausable {
    constructor() {}
    ...
    function test_ERC20_pausedTransfer(address target, uint256 amount) public {
        ...
        assertEq(balanceOf(address(this)), balance_sender, "Transfer while paused altered source balance");
        assertEq(balanceOf(target), balance_receiver, "Transfer while paused altered target balance");

        _unpause();
    }

    // Transfers should not be possible during paused state
    function test_ERC20_pausedTransferFrom(address target, uint256 amount) public {
        ...
        assertWithMsg(r == false, "Tokens transferred while paused");
        assertEq(balanceOf(msg.sender), balance_sender, "Transfer while paused altered source balance");
        assertEq(balanceOf(target), balance_receiver, "Transfer while paused altered target balance");
